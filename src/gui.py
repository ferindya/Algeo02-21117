...
photo_shown = 20
photo_per_row = 2
total_rows = (photo_shown - 1) // photo_per_row + 1
app = wx.App()
...
app.ExitMainLoop()
offset = 8
width = PhotoMaxSize * photo_per_row + offset * (photo_per_row + 1)
height = width * 2
imgpath = ""
image_database = "105_classes_pins_dataset"
...
class panel_one (wx.Panel):
    ...
    def __init__(self, parent):
        ...
        by1 = wx.StaticText(self, -1, "Maggie Zeta Rosida S / 13521117", (100, 50), (300, -1), wx.ALIGN_CENTER)
        by1.SetForegroundColour(wx.WHITE)  # set text color
        font = wx.Font(10, wx.DECORATIVE, wx.NORMAL, wx.NORMAL)
        by1.SetFont(font)

        by2 = wx.StaticText(self, -1, "Ferindya Aulia B / 13521161", (100, 50), (300, -1), wx.ALIGN_CENTER)
        by2.SetForegroundColour(wx.WHITE)  # set text color
        font = wx.Font(10, wx.DECORATIVE, wx.NORMAL, wx.NORMAL)
        by2.SetFont(font)

        by3 = wx.StaticText(self, -1, "Muhammad Habibi Husni / 13521169", (100, 50), (300, -1), wx.ALIGN_CENTER)
        by3.SetForegroundColour(wx.WHITE)  # set text color
        font = wx.Font(10, wx.DECORATIVE, wx.NORMAL, wx.NORMAL)
        by3.SetFont(font)

        # footer
        footer = wx.StaticText(self, -1, "© 2022 bisain All Rights Reserved", (100, 50), (300, -1), wx.ALIGN_CENTER)
    ...
    def match_img(self, event, method):
        ...
        self.matches = matcher.get_match_img_path(imgpath, image_database, topn=photo_shown, method=method)
...
class panel_two (wx.lib.scrolledpanel.ScrolledPanel):
    def __init__(self, parent):
        ...
        # title
        instructLbl = wx.StaticText(self, -1, "Here's top " + str(photo_shown) + " images match!", (100, 50), (300, -1), wx.ALIGN_CENTER)
        instructLbl.SetForegroundColour(wx.WHITE)  # set text color
        ...
        self.imageMatch = []
        for i in range(photo_shown):
            img = wx.Image(PhotoMaxSize, PhotoMaxSize)
            self.imageMatch.append(wx.StaticBitmap(self, wx.ID_ANY, wx.Bitmap(img)))

        for i in range(total_rows):
            sizer = wx.BoxSizer(wx.HORIZONTAL)
            for j in range(photo_per_row):
                if len(parent.drag_and_drop_panel.matches) != 0:
                    filepath = parent.drag_and_drop_panel.matches[i * photo_per_row + j]
                    parent.on_view(filepath, self.imageMatch[i * photo_per_row + j])
                if i * photo_per_row + j < photo_shown:
                    sizer.Add(self.imageMatch[i * photo_per_row + j], 0, wx.ALL, 5)
